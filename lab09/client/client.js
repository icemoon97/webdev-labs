...
const displayUsers = (data) => {
    ...
    const html = users.map(user => `<li id=${user}>${user}</li>`).join("\n");
    qs("#users-list ul").innerHTML = html;
};

const displayChat = (data) => {
    const html = 
    `<div class=${data["username"] === username ? "right" : "left"}>
        <strong>${data["username"] === username ? "You" : data["username"]}</strong>${data["text"]}
    </div>`;

    qs("#chat").insertAdjacentHTML("beforeend", html);
};